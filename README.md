...
**During the group stage**, the bot only tracks matches involving: **Spain, France, England, Argentina, Portugal, United States**
...
CREATE TABLE IF NOT EXISTS fixtures (
  ...
  posted\_schedule INTEGER NOT NULL DEFAULT 0

);

CREATE TABLE IF NOT EXISTS seen\_events (

  fixture\_id  INTEGER NOT NULL,

  event\_key   TEXT    NOT NULL,

  PRIMARY KEY (fixture\_id, event\_key)

);

CREATE TABLE IF NOT EXISTS bot\_state (
...
2. Click the **Triggers** tab and copy your worker URL — it looks like: `https://wc2026-bot.YOUR-NAME.workers.dev`  
3. Open a new browser tab and visit: `https://wc2026-bot.YOUR-NAME.workers.dev/?action=init`  
...
- To force it: go to the D1 Console (Step 4b) and run: `DELETE FROM bot_state WHERE key = 'group_schedule_posted';` Then visit `?action=init` again
...
If unsure, you can look up the exact name by visiting: `https://v3.football.api-sports.io/teams?league=1&season=2026` with your API key in the header `x-apisports-key`.
